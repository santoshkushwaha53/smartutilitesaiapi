...
async function seedDefaultStatesIfEmpty() {
  ...
  return DEFAULT_STATES.length;
}

// GET /api/states
router.get('/', async (_req, res) => {
  await seedDefaultStatesIfEmpty();
...
router.post('/seed', auth, async (req, res) => {
  const results = await Promise.all(
    DEFAULT_STATES.map(s => prisma.state.upsert({ where: { code: s.code }, update: s, create: s }))
  );
